...
function sostavChisla(massivChisel, chislo) {
  ...
  const combine = (index, temp, sum) => {
    if (sum === chislo) return result.push(temp.slice()); // exit if sum found
    if (index >= massivChisel.length) return; // exit if index over
    combine(
      index + 1,
      temp.concat(massivChisel[index]),
      sum + massivChisel[index]
    ); // take value
    combine(index + 1, temp, sum); // omit value
  };
